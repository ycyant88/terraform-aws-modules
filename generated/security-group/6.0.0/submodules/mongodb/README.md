# AWS MongoDB Security Group Terraform Module

Terraform module which creates a pre-configured AWS security group for MongoDB.
...
| ---- | --------- | ------- | -------- | ----------- |
| `mongodb` | 27017 | 27017 | `tcp` | MongoDB |
| `mongodb-config-server` | 27019 | 27019 | `tcp` | MongoDB config server |
| `mongodb-shard` | 27018 | 27018 | `tcp` | MongoDB shard |
...
| <a name="input_preset_ingress_rules"></a> [preset\_ingress\_rules](#input\_preset\_ingress\_rules) | Preset ingress rule definitions for this service. Defaults to the curated catalog set; pass `{}` to disable, or override individual entries | <pre>map(object({<br/>    from_port   = number<br/>    to_port     = number<br/>    ip_protocol = string<br/>    description = optional(string)<br/>  }))</pre> | <pre>{<br/>  "mongodb": {<br/>    "description": "MongoDB",<br/>    "from_port": 27017,<br/>    "ip_protocol": "tcp",<br/>    "to_port": 27017<br/>  },<br/>  "mongodb-config-server": {<br/>    "description": "MongoDB config server",<br/>    "from_port": 27019,<br/>    "ip_protocol": "tcp",<br/>    "to_port": 27019<br/>  },<br/>  "mongodb-shard": {<br/>    "description": "MongoDB shard",<br/>    "from_port": 27018,<br/>    "ip_protocol": "tcp",<br/>    "to_port": 27018<br/>  }<br/>}</pre> | no |
