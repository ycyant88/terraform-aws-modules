# AWS Zipkin Security Group Terraform Module

Terraform module which creates a pre-configured AWS security group for Zipkin.
...
module "zipkin_security_group" {
  ...
  name        = "zipkin"
  description = "Security group for zipkin"
  vpc_id      = "vpc-xxxxxxxx"
...
| ---- | --------- | ------- | -------- | ----------- |
| `zipkin-admin` | 9990 | 9990 | `tcp` | Zipkin Admin port collector |
| `zipkin-admin-query` | 9901 | 9901 | `tcp` | Zipkin Admin port query |
| `zipkin-admin-web` | 9991 | 9991 | `tcp` | Zipkin Admin port web |
| `zipkin-query` | 9411 | 9411 | `tcp` | Zipkin query port |
| `zipkin-web` | 8080 | 8080 | `tcp` | Zipkin web port |
...
| <a name="input_preset_ingress_rules"></a> [preset\_ingress\_rules](#input\_preset\_ingress\_rules) | Preset ingress rule definitions for this service. Defaults to the curated catalog set; pass `{}` to disable, or override individual entries | <pre>map(object({<br/>    from_port   = number<br/>    to_port     = number<br/>    ip_protocol = string<br/>    description = optional(string)<br/>  }))</pre> | <pre>{<br/>  "zipkin-admin": {<br/>    "description": "Zipkin Admin port collector",<br/>    "from_port": 9990,<br/>    "ip_protocol": "tcp",<br/>    "to_port": 9990<br/>  },<br/>  "zipkin-admin-query": {<br/>    "description": "Zipkin Admin port query",<br/>    "from_port": 9901,<br/>    "ip_protocol": "tcp",<br/>    "to_port": 9901<br/>  },<br/>  "zipkin-admin-web": {<br/>    "description": "Zipkin Admin port web",<br/>    "from_port": 9991,<br/>    "ip_protocol": "tcp",<br/>    "to_port": 9991<br/>  },<br/>  "zipkin-query": {<br/>    "description": "Zipkin query port",<br/>    "from_port": 9411,<br/>    "ip_protocol": "tcp",<br/>    "to_port": 9411<br/>  },<br/>  "zipkin-web": {<br/>    "description": "Zipkin web port",<br/>    "from_port": 8080,<br/>    "ip_protocol": "tcp",<br/>    "to_port": 8080<br/>  }<br/>}</pre> | no |
