...
variable "source_hash" {
  description = "Triggers updates like etag but works around its encryption limitations. Set with filemd5(\"path/to/source\"); the value is only stored in state and never sent to AWS"
  type        = string
  default     = null
}
